...
export const processFile = async (filePath, relativePath, config) => {
  try {
    ...
    if (config.detectBinary) {
      isBinary = await isBinaryFile(filePath, config);
    }
...
function formatOutput(fileContents, config) {
  ...
  switch (config.outputFormat) {
    case "json":
      output = formatJSON(fileContents, config);
      break;

    case "markdown":
      output = formatMarkdown(fileContents, config);
      break;

    case "csv":
      output = formatCSV(fileContents, config);
      break;

    case "html":
      output = formatHTML(fileContents, config);
      break;

    case "xml":
      output = formatXML(fileContents, config);
      break;

    case "text":
    default:
      output = formatText(fileContents, config);
      break;
  }
